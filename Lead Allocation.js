function copyLeadsToOGTa() {
  var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var leadsSheet = spreadsheet.getSheetByName("sheet name");
  var leadsOGVSheet = SpreadsheetApp.openByUrl("link").getSheetByName("oGTa");
  ...
  for (var i = 0; i < dataValues.length; i++) {
    ...
    if (leadData[14] === "oGTa") {
      // Check if "Our choice" column value is "oGTa" or "oGTe"
      ...
      if (!leadExists) {
        ...
        leadsOGVSheet
          ...
          .setValues([leadInfo]);
        Logger.log("Lead '" + leadName + "' copied to Leads oGT sheet.");
      } else {
